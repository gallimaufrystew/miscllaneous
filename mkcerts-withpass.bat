...
set CN="Server Cert"
%openssl% req  -passout pass:%PASSWD% -config ca.cnf -keyout sk.pem -out sr.pem -newkey rsa:4096
REM  Sign request: end entity extensions
%openssl% x509 -req -passin pass:%PASSWD% -in sr.pem -CA int_ca2.crt -CAkey ik_2.pem -days 3600 -extfile ca.cnf -extensions usr_cert -CAcreateserial -out server.crt


REM  Client certificate
set CN="Client Cert"
%openssl% req -passout pass:%PASSWD% -config ca.cnf -keyout ck.pem -out cr.pem -newkey rsa:4096 
REM  Sign using intermediate C
%openssl% x509 -req -passin pass:%PASSWD% -in cr.pem -CA int_ca2.crt -CAkey ik_2.pem -days 3600 -extfile ca.cnf -extensions usr_cert -CAcreateserial -out client.crt
